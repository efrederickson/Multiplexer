...
UITextField *currentMonitoringTextField = nil;
...
inline void sendKeyEventReturnToTopApp()
{
    CFNotificationCenterPostNotification(CFNotificationCenterGetDistributedCenter(), CFSTR("com.efrederickson.reachapp.simulatekeypress"), NULL, (__bridge CFDictionaryRef) @{
        @"key": @"",
        @"isReturn": @YES,
...
void handleKeyEvent(CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo)
{
    if (isTopApp)
    {
        ...
        if ([dict objectForKey:@"isReturn"] != nil)
        {
            BOOL isReturn = [dict[@"isReturn"] boolValue];
            if (isReturn)
            {
...
{
    if (topAppWantsKeyboardEvents && [text isEqual:@"\n"])
       sendKeyEventReturnToTopApp();
    else
        %orig;
}
%end
                    ...
                    CFDictionaryRef userInfo)
{
    if (!isTopApp && overrideDisplay) // bottom app
    {
        ...
        if ([name2 isEqual:@"com.efrederickson.reachapp.topAppWantsKeyboardEvents"])
        {
            ...
            if ([[UIWindow.keyWindow firstResponder] isKindOfClass:[UITextView class]] == NO)
            {
                currentMonitoringTextField = [[UITextField alloc] initWithFrame:CGRectMake(0, 0, UIScreen.mainScreen.bounds.size.width, 30)];
                currentMonitoringTextField.alpha = 0;
                [[UIWindow keyWindow] addSubview:currentMonitoringTextField]; // hue hue hue
                [currentMonitoringTextField becomeFirstResponder]; // show the keyboard!
            }
        }
        else
        {
            ...
            if (currentMonitoringTextField)
            {
                [currentMonitoringTextField resignFirstResponder];
                [currentMonitoringTextField removeFromSuperview];
                currentMonitoringTextField = nil;
            }
        }
    }
}
